...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (action === "approve") {
    const conflict = findConflict(booking.resource, booking.date, booking.startTime, booking.endTime, booking.id);
    if (conflict) {
    ...
    store.update(booking.id, { status: "confirmed" });
    // TODO: e-mail žadateli "schváleno" (text ve specifikaci)
  } else if (action === "reject") {
    store.update(booking.id, { status: "rejected" });
    // TODO: e-mail žadateli "zamítnuto" (text ve specifikaci)
  } else {
    return NextResponse.json({ error: "neznámá akce" }, { status: 400 });
  }

  return NextResponse.json(store.all().find((b) => b.id === params.id));
}
